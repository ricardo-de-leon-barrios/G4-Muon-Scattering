...
// --------------------------------------------------------------
  ...
  if ( track->GetDefinition() == G4OpticalPhoton::OpticalPhotonDefinition() )
  {
    ...
    if ( cerenProc == "Cerenkov" && volumName == "pmt_geo" )
    {
      ...
      wlSpect = pmtSD->ProcessHits(step, track->GetGlobalTime());

      //if ( pmtSD->ProcessHits(step, track->GetGlobalTime()) > 0 )
      if ( wlSpect > 0 )
      {
        ...
        if ( fEventAction->muonOk )
        {
          fEventAction->fRunAction->histRun->cerenPhoElect(0,wlSpect);
          fEventAction->fRunAction->histRun->meanPulse(0, trackTime);

        }
        else if ( fEventAction->elecOk )
        {
          fEventAction->fRunAction->histRun->cerenPhoElect(1,wlSpect);
          fEventAction->fRunAction->histRun->meanPulse(1, trackTime);
        }
        else if ( fEventAction->gammOk )
        {
          fEventAction->fRunAction->histRun->cerenPhoElect(2,wlSpect);
          fEventAction->fRunAction->histRun->meanPulse(2, trackTime);
        }
        else if ( fEventAction->neutOk )
        {
          fEventAction->fRunAction->histRun->cerenPhoElect(3,wlSpect);
          fEventAction->fRunAction->histRun->meanPulse(3, trackTime);
        }
        else
        {
          fEventAction->fRunAction->histRun->cerenPhoElect(4,wlSpect);
          fEventAction->fRunAction->histRun->meanPulse(4, trackTime);
        }

      fEventAction->fRunAction->histRun->cerenPhoElect(5,wlSpect);
